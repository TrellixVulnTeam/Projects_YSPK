...
class WeighingSchemes:
    ...
    parameter and input from the user, then the week number in which the given date
    is located will be calculated and hen all days of said week with the 
    calculated week:number are structered into a list of dates, 
    ...
    def frequencyAnalysisWithParticularizedDateSummedUpToWeek(self, date, year):
        result = dict()
        for dateString in self.filterObject.getWeekDaysOfAWeekForASpecificDate(date, year):
            newDate = dateString.split("-")[1] + " " + dateString.split("-")[0]
            newYear = dateString.split("-")[2]
            print(newDate, newYear)
            resultOfQuery = self.frequencyAnalysisWithParticularizedDate(newDate, newYear)
